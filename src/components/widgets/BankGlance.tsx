  ...
  return (
    <div className="panel flex h-full flex-col p-5">
      ...
      <div className="grid flex-1 grid-cols-2 gap-x-6 gap-y-4 lg:grid-cols-4">
        <div>
          <p className="text-[11px] uppercase tracking-wide text-faint">{t("w.balance")}</p>
          <p className={`num mt-1 text-2xl font-semibold tracking-tight ${privacy ? "money-blur" : ""}`}>{eur(balance)}</p>
        </div>
        <div>
          <p className="text-[11px] uppercase tracking-wide text-faint">{format(new Date(), "MMMM", { locale: dfLocale(lang) })}</p>
          ...
          <p className={`num mt-1 text-xs text-faint ${privacy ? "money-blur" : ""}`}>≈ {eur(Math.round(subsMonthly))}/{lang === "de" ? "Mon." : "mo"} {t("w.recurring")}</p>
        </div>
        <div className="col-span-2">
          <p className="mb-1.5 text-[11px] uppercase tracking-wide text-faint">{t("w.latest")}</p>
          <ul className="flex flex-col gap-1">
            {bank.transactions.slice(0, 3).map((t) => (
              <li key={t.id} className="flex items-center gap-2 text-[13px]">
                ...
                <span className="num shrink-0 text-xs text-faint">{format(parseISO(t.date), lang === "de" ? "d. MMM" : "MMM d", { locale: dfLocale(lang) })}</span>
