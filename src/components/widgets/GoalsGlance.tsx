    ...
    <div className="panel flex h-full flex-col p-5">
      ...
      {goals.length === 0 && (
        <Link href="/goals" className="flex flex-1 flex-col items-center justify-center gap-1.5 py-6 text-center">
          <p className="text-sm font-medium">{t("w.noGoals")}</p>
          <p className="max-w-[28ch] text-xs text-muted">{t("w.noGoalsHint")}</p>
        </Link>
      )}
            ...
            <li key={g.id}>
              <div className="mb-1.5 flex items-baseline justify-between gap-3">
                ...
                <span className="num shrink-0 text-xs text-faint">
                  {daysLeft !== null && daysLeft >= 0 ? t("w.daysLeft").replace("{n}", String(daysLeft)) : `${done}/${g.milestones.length}`}
                </span>
              </div>
